...
public sealed class CacheVideoRedis : ICacheVideo
{
    ...
    public CacheVideoRedis(string connectionString, TimeSpan ttl)
    {
        _ttl = ttl;
    ...
    public async Task RemoverVideosAsync(string userId, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        await Database.KeyDeleteAsync(VideoCacheKeys.Videos(userId));
    }

    public async Task RemoverVideoAsync(string userId, Guid videoId, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
